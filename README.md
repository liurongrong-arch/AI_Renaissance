...
└───────┬──────┬───────┬───────┬───────┬───────┬───────┬──────────┘
   ...
   ┌──┴────────┴────────┴────────┴────────┴────────┴────────┴───┐
   │                   data_sources（数据层）                       │
   │              开发3组统一封装，纯 Python 接口                     │
   │          东方财富 · 腾讯自选股 · 巨潮 · Wind                    │
   └──────────────────────────────────────────────────────────────┘
```

**8 个 Agent + N 个 Skill + 统一数据层，从信号到认知：**
...
- **数据层**：`data_sources/`，纯 Python 类/函数，Agent 只管调接口不管数据从哪来
...
| 开发3组（数据） | 过去，未来 | 统一封装所有数据源（data_sources/），让 Agent 只管分析不管数据从哪来 |
...
- **数据归开发3组** — Agent 只调 `data_sources/` 接口，不直接调 API
