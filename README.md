...
| 模块 | 当前状态 |
|------|----------|
| Agent / Signal 基础框架 | 已有 `BaseAgent`、统一 `Signal`、专家 Agent 目录和主流程注册表 |
...
| CI | 已接入依赖检查、pytest、compileall 和轻量 Agent 冒烟检查 |
| AgentScope | 尚未正式接入；当前测试只守最低接入口，不绑定未来 AgentScope 生命周期 |
...
测试包含基础 smoke test 和轻量 Agent 冒烟检查。后者只确认注册的专家 Agent 可以离线跑通并返回标准 `Signal`，不验证业务判断是否正确。
