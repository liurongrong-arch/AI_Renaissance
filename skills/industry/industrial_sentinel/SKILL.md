...
# Industrial Sentinel V4.5 | 产业链中观分析框架
...
> 本 skill 位于 [AI_Renaissance](https://github.com/duolongworld/AI_Renaissance) 项目 `skills/industry/industrial-sentinel/` 目录下，由 Agent 通过 `runtime.py` 调用。
...
**陷阱0：不要在框架里建数据抓取引擎。**
这是本框架最核心的设计约束。永远不要加 `data_fetcher.py`、`source_trust_engine.py` 或任何硬编码数据源。
框架只负责"搜什么、怎么校验"，搜索由 AI Agent 用自身工具完成，用户确认后回填。
硬编码数据源 = 技术债（API 改字段/网站反爬 → 全框架报废）。
